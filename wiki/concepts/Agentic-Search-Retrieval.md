...
论文[SAAS: Self-Aware Reinforcement Learning for Over-Search Mitigation in Agentic Search](https://arxiv.org/abs/2605.29796)提出了Agentic Search的**过度搜索问题**：Agent在多跳推理中容易陷入无限搜索循环，消耗大量Token却无法收敛到答案。

**SAAS核心方法**：通过强化学习训练Agent的**自我感知能力**，让Agent学会判断"当前信息是否足够回答问题"，从而在适当时候停止搜索。这直接回应了[Agent-Cost-Crisis-2026](../sources/Agent-Cost-Crisis-2026.md)中讨论的成本失控问题。

### 2026-07-19 更新：GRASP — 粒度感知搜索策略

[GRASP: GRanularity-Aware Search Policy for Agentic RAG](https://arxiv.org/abs/2607.10463)提出用强化学习（RL）训练Agent自适应协调多种检索工具（语义搜索、关键词搜索、段落阅读），在推理不同阶段动态选择搜索粒度。非单调推理中使用 RL 优化检索召回率和准确率。

同时，[GrepSeek](https://arxiv.org/abs/2605.29307)论文提出训练搜索Agent直接与语料交互（grep-style），是对DCI范式的工程化实现——将"从语料中提取"具体化为可训练的搜索行为。
