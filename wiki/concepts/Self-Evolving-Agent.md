...
[SEED: Self-Evolving On-Policy Distillation for Agentic Reinforcement Learning](https://arxiv.org/abs/2607.14777) 提出将 LLM 作为交互式 Agent 训练的**自进化在线策略蒸馏**方法：
...
**与已有三篇论文的关系**：SEED 不同于 MLEvolve（算法发现）、EvoDS（技能学习）和 SePO（prompt 优化），而是从 RL 训练层面切入——不是让 Agent 在推理时自进化，而是在**训练阶段**通过 on-policy distillation 实现自进化。这一视角补充了 Self-Evolving Agent 从推理时自进化到训练时自进化的完整图景。
