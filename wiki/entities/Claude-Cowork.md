...
### 2026-07-21 更新：Cowork使用数据分析（1.2M会话样本）

2026年7月，Anthropic分析了2026年5月11-31日期间120万条匿名聚合Claude Cowork会话，按20种工作类别分类，揭示了AI在工作场景中的实际使用模式：

**使用分布：**
- **业务流程与运营（33.4%）**：最大类别，包括零散更新汇总为单一报告、构建入职检查清单、对账等。跨财务、HR、行政等多角色通用
- **内容创作与文案撰写（16.4%）**：草稿、幻灯片、帖子、方案等合成密集型商务沟通。跨营销、公关、BD、项目管理等角色
- **软件开发（8.7%）**
- **DevOps与基础设施（7%）**
- **研究与情报（6.4%）**
- **数据分析与BI（5.8%）**
- **文档处理与提取（4.1%）**
- **销售与营收运营（4%）**
- 其余类别均低于4%，包括个人助理（3.8%）、教育（2.4%）、会议智能（1.8%）

**核心发现：** 约一半使用量为"围绕工作的工作"（the work around the work）——这些任务是广泛职位的组成部分，但很少是某个人的核心职责。这一发现验证了Cowock降低"工作摩擦力"的产品定位——不是替代核心专业技能，而是消除围绕核心工作的重复性事务。

**启示：** 对于AI Agent产品设计，应优先覆盖跨角色的通用场景（报告生成、数据整理、文档处理），而非过早深入特定垂直领域。

### 2026-07-21 更新：Anthropic营销团队Cowork实践

Anthropic营销运营团队（Ian Chan和Annabel Custer）分享了Cowork在企业营销场景的具体实践：

**自动化周报生成：** Ian过去每周花1-2天手动汇总营销指标报告，涉及Salesforce、HubSpot、Google Analytics等多个平台的数据拉取和格式化。通过Cowork工作流，压缩为数小时完成。

**自动化活动搭建：** Annabel过去每搭建一个新活动需要依次操作Salesforce、HubSpot、Swoogo和邮件工具。Cowork工作流将这一串行流程自动化，减少人工操作。

**关键启示：** 企业AI自动化的最优切入点是"多系统数据整合+重复性流程"——这些任务AI做得好且出错了风险低。Cowork的Connector系统（连接外部数据源）是解决"系统不集成"问题的核心组件。
