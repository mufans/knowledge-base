...
### 2026-05-17 更新：Agent View 功能

2026年5月，Claude Code 引入 **Agent View** 功能，解决多Agent并行管理问题：

- **核心痛点**：此前并行运行多个Claude Code Agent需要管理多个终端tab或tmux窗口，心智负担重
- **Agent View 方案**：一个界面统一管理所有Claude Code session
  - 启动新Agent并发送到后台
  - 一览哪些Agent在等待用户输入、哪些仍在运行、哪些已完成
  - 只在Claude需要用户时介入
- **设计理念**：从"用户管理多个终端"转向"一个Dashboard管理多个Agent"，降低了并行Agent的心智负担
- **与源码中Subagent系统的关系**：Agent View是前端展示层（Ink TUI），底层仍然是 `src/tasks/` 中的 LocalAgentTask/DreamTask/RemoteAgentTask 系统
- **参考**：[Agent view in Claude Code](https://claude.com/blog/agent-view-in-claude-code)

关联：
- [Anthropic-Agent-API](Anthropic-Agent-API.md) — API层Agent能力
- [OpenCode-Agent](OpenCode-Agent.md) — 开源替代方案

### 2026-07-20 更新：Bun/Rust 运行时迁移 (v2.1.181+)

2026年6月17日，Claude Code v2.1.181 起切换到 **Rust 重写的 Bun 运行时**（Bun v1.4.0-canary）：

- **运行时变更**：从 Zig 实现的 Bun 迁移到 Rust 实现的 Bun，Simon Willison 通过 `strings ~/.local/bin/claude | grep -Eo 'src/[a-z_./-]+\\.rs'` 验证了 563 个 `.rs` 源码文件的存在
- **性能提升**：Linux 上启动速度提升约 10%，macOS 上几乎无感知
- **版本号**：内置的 Bun 版本为 v1.4.0（当前公开最新为 v1.3.14），证明是 canary 预览版本
- **核心团队评价**："Boring is good"——Rust 重写对用户来说是无感的，恰恰说明迁移成功

**核心意义**：Bun 从 Zig 到 Rust 的重写反映了运行时层面的技术路线选择。对 Claude Code 用户来说这一变化基本透明，但对关注 [OpenClaw](OpenClaw.md) 等 Agent 工具链的开发者而言，Rust 在 Agent 基础设施层的渗透趋势值得关注。

参考：[Simon Willison: Claude Code uses Bun written in Rust now](https://simonwillison.net/2026/Jul/19/claude-code-in-bun-in-rust/)
## 自评
| 维度 | 分数 | 权重 | 加权 |
|------|------|------|------|
| 摘要质量 | 9 | 0.25 | 2.25 |
...
> 评分说明：摘要涵盖架构全貌和启动流程细节；技术深度来自反编译源码级别的分析；相关性极高（Claude Code是核心关注工具）；原创性体现在源码级模式提炼；格式规范完善。
