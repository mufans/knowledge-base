...
### OpenClaw 独有
- **Subagent 系统**：编码任务可委派给 Codex/Claude Code/Pi Agent，session fork + 自动 announce
- **Heartbeat 机制**：定时主动检查（邮件/日历/天气等），支持活跃时段控制
- **Cron 系统**：croner 调度 + isolated session + SQLite Task Registry + 多步骤 TaskFlow
- **Browser 控制**：原生 browser tool（snapshot/act），CDP 协议，多 profile，SSRF 防护
- **Node 连接**：Android/iOS/macOS companion app（蓝牙/Wi-Fi/Tailscale）
- **TTS/STT**：内置语音合成（ElevenLabs 等）和识别（Deepgram、Whisper 等）
- **110+ 扩展**：25+ channel、40+ provider、15+ tool 扩展
- **MCP Server**：暴露插件工具给外部 Agent（如 Claude Code）
- **安全扫描**：Skill 安装前自动检测 exec/eval/外泄/混淆
- **轻量级**：Node.js ESM 部署，Rollup chunk splitting
...
1. **Heartbeat**：比 Nudge 更灵活，可自定义检查内容 + 活跃时段控制
2. **双文件记忆**：日记+长期分离，MEMORY.md 在 subagent/cron 中隔离加载
3. **Subagent 系统**：编码任务委派是刚需，session fork 共享 prefix cache
4. **人格系统**：SOUL.md + IDENTITY.md + AGENTS.md 三层人格定义
5. **轻量部署**：Node.js ESM，`npm install -g` 即可
6. **扩展生态**：110+ 内置扩展，plugin SDK 支持第三方开发
7. **安全机制**：文件边界检查、环境变量保护、Skill 安装扫描、SSRF 防护
8. **TaskFlow**：多步骤任务编排（SQLite 持久化），超越简单 cron
