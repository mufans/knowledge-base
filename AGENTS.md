...
- 提到已有wiki条目时使用交叉链接：
  ...
  - 跨目录：`[Codex-context](../entities/Codex-context.md)`
...
章节按内容需要出现，不得用单一模板填充空泛段落。页面必须包含 2–5 个英文技术
标签；事实、推断和假设分别标为 `Fact`、`Inference`、`Hypothesis`。自评分可作为
历史元数据保留，但不得作为发布门禁或默认排序依据。
...
- ✅ 数字、版本、性能、API 声明必须 100% 可追溯，其他重要事实引用覆盖率 ≥ 90%
- ✅ 明确记录限制、反例或反方证据，并说明相对已有页面的信息增量
- ✅ 中文输出，技术术语保留英文原文
...
| 实体 | entities/ | 具体的项目/工具/产品 | OpenClaw、Codex-mem、ml-intern |
...
**与AGENTS.md的区别**：schema是结构规则（怎么做），purpose是方向意图（做什么）。

## Ingest流程（摄取新知识）

1. 读取 `purpose.md` 确认方向匹配
2. 将外部内容转换为版本化 `Signal`，缺失 URL、采集时间、hash 或稳定 ID 时拒绝
3. **增量检查**：比较内容 hash、语义重复和已有页面，不以“文件已处理”直接跳过更新判断
4. **已有页面匹配与更新**：
   - 对inbox中每条内容，扫描 `wiki/` 目录检查是否已有相关页面（匹配方式：项目名/概念名/工具名）
   - **匹配到已有页面**：读取已有页面，判断新内容是否有实质性更新（新特性、新版本、新数据、新事件等）
     - 有实质性更新 → 合并到已有页面，在对应章节追加新内容，标注更新日期（如 `### 2026-05-01 更新`）
     - 无实质性更新（只是重复提及） → 跳过，记录"已覆盖，无新内容"
   - **未匹配到已有页面**：有明确增量且门禁通过才创建；0 个新页面是正常成功结果
5. 生成 Research Dossier 和 `WikiCandidate`，由独立 reviewer 检查引用覆盖率、正反证据、重复、purpose 与链接
6. reviewer 输出 `publish`、`needs_human` 或 `reject`；任何 validation error 必须可观察
7. 发布时建立交叉链接，并更新对应 `index.md` 和根 `log.md`
   - 新建页面：`YYYY-MM-DD | ingest | 来源 | 创建wiki页面名（citation coverage X%）`
   - 更新页面：`YYYY-MM-DD | update | raw/inbox/文件名 | 更新wiki页面名（合并了XX新内容）`
...
- 标记过时的内容
- 运行 `python scripts/check_quality.py` 检查所有 wiki 页面质量
...
3. 根文件 → docs/（AGENTS.md、README.md、log.md、purpose.md）
