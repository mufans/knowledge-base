...
- `ngrok` 只通过官方 `ngrok service install/start/restart` 管理。配置必须把 upstream 指向 `http://127.0.0.1:<port>`，内联 GitHub OAuth 和拒绝非 owner 的规则，并以 `0600` 保存。状态查询通过 loopback `127.0.0.1:4040` 的 ngrok agent API 读取，不伪造官方不存在的 `service status` 命令。
