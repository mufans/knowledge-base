...
- macOS LaunchAgent 只负责 loopback Dashboard 自启动；ngrok 不创建自定义 plist。Dashboard LaunchAgent 以 `0600` 保存，并注入 `DASHBOARD_HOME`、稳定开发域名对应的 `DASHBOARD_REMOTE_HOST` 和强随机 `DASHBOARD_ORIGIN_CREDENTIAL`。
...
python -m opportunity_os.deployment ngrok-config \
  ...
  --owner-github-id-file /absolute/private/path/github-provider-user-id \
  --origin-credential-file /absolute/private/path/dashboard-origin \
  --remote-host owner.ngrok-free.app \
...
生产应用时在确认 dry-run 后显式增加 `--apply`。三个输入文件必须是绝对路径、非符号链接、普通文件且权限为 `0600` 或更严格；CLI 直接安全读取，秘密不会出现在 argv 或 shell 替换命令中。GitHub owner 使用稳定的数字 `provider_user_id`，不依赖可能私密的邮箱；origin credential 使用至少 32 字节随机数编码为 43 个以上 URL-safe 字符。

`ngrok-config` 结构化生成完整 v3 YAML（模板仅供审计），强制绑定账户分配的稳定 HTTPS 开发域名，并以 `0600` 原子写入；`ngrok service install` 前适配器会执行本地严格 schema 校验与官方 `ngrok config check --config`。Dashboard LaunchAgent 使用同一纯 hostname（不含 scheme/path），使 Host allowlist 与 ngrok origin header 同时闭环；重启服务不会改变入口。
